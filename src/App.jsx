...
// Firebaseはオブジェクト/配列を変換するので正規化が必要
function toArr(val) {
  if (!val) return [];
...
function normalizeGame(g) {
  ...
  const stacks = {};
  if (g.stacks) {
    Object.keys(g.stacks).forEach(k => {
      stacks[Number(k)] = toArr(g.stacks[k]);
    });
  }
  // legBets の各色を配列に
  const legBets = {};
  if (g.legBets) {
    Object.keys(g.legBets).forEach(k => {
      legBets[k] = toArr(g.legBets[k]);
    });
  }
  // raceBets
  const raceBets = {
    win: toArr(g.raceBets?.win).filter(x => x && typeof x === 'object'),
    lose: toArr(g.raceBets?.lose).filter(x => x && typeof x === 'object'),
  };
  // log
  const log = toArr(g.log);
  // usedDice
  const usedDice = toArr(g.usedDice);
  // players
  const players = toArr(g.players);
  // positions
  const positions = g.positions || {};
  return { ...g, stacks, legBets, raceBets, log, usedDice, players, positions };
}
...
function rollDice(state) {
  const remaining = NORMAL_CAMELS.filter(c => !state.usedDice.includes(c));
  const canCrazy = !state.crazyDiceUsed;
  const pool = remaining.map(c => ({ type: "normal", camel: c }));
  if (canCrazy) pool.push({ type: "crazy" });
  if (!pool.length) return null;
  const pick = pool[Math.floor(Math.random() * pool.length)];
  const steps = Math.floor(Math.random() * 3) + 1;
  if (pick.type === "crazy") {
    return { camel: CRAZY_CAMELS[Math.floor(Math.random() * 2)], steps, isCrazy: true };
  }
...
function moveCamel(state, camel, steps, crazy) {
  ...
  // stacks を正規化
  const stacks = {};
  Object.keys(ns.stacks).forEach(k => {
    stacks[Number(k)] = toArr(ns.stacks[k]);
  });
  ns.stacks = stacks;
...
function getRankedNormal(state) {
  const stacks = {};
  Object.keys(state.stacks).forEach(k => { stacks[Number(k)] = toArr(state.stacks[k]); });
  return [...NORMAL_CAMELS].sort((a, b) => {
    const pa = state.positions[a] ?? 0, pb = state.positions[b] ?? 0;
    if (pa !== pb) return pb - pa;
    const sa = stacks[pa] || [], sb = stacks[pb] || [];
    return sb.indexOf(b) - sa.indexOf(a);
  });
}

function _scoreRaceBets(ns) {
  const ranked = getRankedNormal(ns);
  const leader = ranked[0], last = ranked[ranked.length - 1];
  let wi = 0;
  ns.raceBets.win.forEach(({ player, camel }) => {
    const gain = camel === leader ? RACE_BET_PAYOUTS[Math.min(wi++, RACE_BET_PAYOUTS.length-1)] : -1;
    ns.coins[player] = (ns.coins[player] || 0) + gain;
    ns.log.unshift(`🏆 ${player}: 優勝ベット ${gain>0?"+":""}${gain}コイン`);
  });
  let li = 0;
  ns.raceBets.lose.forEach(({ player, camel }) => {
    const gain = camel === last ? RACE_BET_PAYOUTS[Math.min(li++, RACE_BET_PAYOUTS.length-1)] : -1;
    ns.coins[player] = (ns.coins[player] || 0) + gain;
...
function scoreEndOfLeg(state) {
  ...
  NORMAL_CAMELS.forEach(camel => {
    const bets = toArr(ns.legBets[camel]);
...
function CamelStack({ stack }) {
  const arr = toArr(stack);
  return (
    <div style={{ display: "flex", flexDirection: "column-reverse", alignItems: "center" }}>
      {arr.map((c, i) => (
        <div key={c} style={{ fontSize: i === arr.length-1 ? 18 : 14, transform: `translateY(${i*-2}px)`, textShadow: "0 2px 4px rgba(0,0,0,0.6)", lineHeight: 1 }}>
          {CAMEL_EMOJI[c]}
        </div>
      ))}
    </div>
  ...
  return (
    <div style={{ display: "flex", flexWrap: "wrap", gap: 10 }}>
      {NORMAL_CAMELS.map(camel => {
        const bets = toArr(state.legBets[camel]);
          ...
          <div key={camel} style={{ background: palette.cardBg, border: `1px solid ${palette.cardBorder}`, borderRadius: 12, padding: "10px 14px", display: "flex", flexDirection: "column", alignItems: "center", gap: 4, minWidth: 70 }}>
            ...
            <div style={{ display: "flex", gap: 3, flexWrap: "wrap", justifyContent: "center" }}>
              {LEG_BET_TILES.map((tile, idx) => {
                const taken = idx < takenCount, isNext = idx === takenCount;
                return (
...
export default function App() {
  ...
  const joinGame = async () => {
    ...
    setMyName(name);
    const latest = normalizeGame(gameRef.current);
    let players = latest?.players || [];
    if (!players.includes(name)) players = [...players, name];
    let current = latest ? { ...latest, players } : initGame(players);
    if (!current.coins) current.coins = {};
  ...
  const act = async fn => {
    ...
    const latest = normalizeGame(snapshot.val()) || normalizeGame(gameRef.current);
  ...
  const handleLegBet = camel => act(g => {
    ...
    if (!ns.legBets) ns.legBets = {};
    if (!ns.legBets[camel]) ns.legBets[camel] = [];
    else ns.legBets[camel] = toArr(ns.legBets[camel]);
  ...
  const handleRaceBet = (type, camel) => act(g => {
    const ns = JSON.parse(JSON.stringify(g));
    if (!ns.raceBets) ns.raceBets = { win: [], lose: [] };
    ns.raceBets[type] = toArr(ns.raceBets[type]).filter(x => x && typeof x === 'object');
  ...
  const usedDice = toArr(game.usedDice);
  const remainingNormal = NORMAL_CAMELS.filter(c => !usedDice.includes(c));
  const crazyRemaining = !game.crazyDiceUsed;
  const rankedNormal = getRankedNormal(game);
  ...
  const allWinBets = toArr(game.raceBets?.win).filter(x => x && typeof x === 'object');
  const allLoseBets = toArr(game.raceBets?.lose).filter(x => x && typeof x === 'object');
    ...
    <div style={{ minHeight: "100vh", background: `linear-gradient(170deg, ${palette.night} 0%, #1a0d40 60%, ${palette.dusk} 100%)`, fontFamily: "'Segoe UI', sans-serif", color: "white", padding: "16px", display: "flex", flexDirection: "column", gap: 14, maxWidth: 740, margin: "0 auto" }}>
        ...
        <>
          ...
          <div style={ss}>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginBottom: 14 }}>
              {NORMAL_CAMELS.map(c => {
                const myCount = allWinBets.filter(b => b.player===myName && b.camel===c).length;
                return (
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
              {NORMAL_CAMELS.map(c => {
                const myCount = allLoseBets.filter(b => b.player===myName && b.camel===c).length;
                return (
      ...
      <div style={ss}>
        ...
        <div style={{ maxHeight: 130, overflowY: "auto", display: "flex", flexDirection: "column", gap: 3 }}>
          {toArr(game.log).slice(0, 18).map((l, i) => (
